...
def control_led(command: LEDCommand, db: Session = Depends(get_db)):
    ...
    if led_type not in ["internal", "external"]:
        raise HTTPException(400, "led_type deve ser 'internal' ou 'external'")

    if command.status.upper() not in ["ON", "OFF"]:
        raise HTTPException(400, "status deve ser 'ON' ou 'OFF'")
    ...
    if not success:
        raise HTTPException(500, "Erro ao controlar LED")
...
def led_on(led_type: str, raspberry_id: int = Query(1), db: Session = Depends(get_db)):
...
def led_off(led_type: str, raspberry_id: int = Query(1), db: Session = Depends(get_db)):
...
def get_led_status(raspberry_id: int = Query(1), db: Session = Depends(get_db)):
    device = db.query(DeviceStatus).filter(DeviceStatus.raspberry_id == raspberry_id).first()
    if not device:
...
def get_led_history(
    raspberry_id: Optional[int] = Query(None),
    led_type: Optional[str] = Query(None),
...
def get_all_devices_status(db: Session = Depends(get_db)):
    ...
    # Use from_orm para desserializar net_ifaces JSON em lista
    return [DeviceStatusResponse.from_orm(d) for d in devices]
